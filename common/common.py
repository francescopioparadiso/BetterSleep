...
def json_error_page(status, message, traceback, version):
    ...
    return json.dumps({"status": status_code, "error": message})
...
def init_mqtt_helper(service_instance, mqtt_info, logger=None, clientid_fallback=None):
    ...
    try:
        ...
        if hasattr(service_instance, "topic_subscribe_raw"):
            ...
            logger.info(
                "MQTT client initialized and subscribed to %s",
                service_instance.topic_subscribe_raw,
            )
            ...
            logger.info(
                "MQTT client initialized and subscribed to %s",
                service_instance.topic_subscribe_list,
            )
        else:
